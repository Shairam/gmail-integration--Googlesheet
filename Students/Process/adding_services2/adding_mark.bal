...
function sendNotification() returns (json) {
    ...
    foreach value in values {
        ...
        if (i > 0) {
            int marks = check <int> value[0];
            string studentName = value[1];
            string studentEmail = value[2];
            log:printInfo("hello "+value[1]);

             out = insertData(studentName, marks, i, studentEmail);

              if(marks > 80) {
                 sendEmail(studentEmail,studentName,getCustomEmailTemplate(studentName, marks, "First Class"));
            }

            else if(marks > 60) {
                 sendEmail(studentEmail,studentName,getCustomEmailTemplate(studentName, marks,"Second Upper clas"));
            }

            else if(marks > 40) {
                 sendEmail(studentEmail,studentName,getCustomEmailTemplate(studentName, marks, "Second Class"));
            }
            else {
                sendEmail(studentEmail,studentName,getCustomEmailTemplate(studentName, marks,""));
            }
